    ...
    with st.sidebar:
        ...
        with tab_add:
            with st.form("add_habit_form", clear_on_submit=True):
                ...
                new_habit_desc = st.text_area("Description (Optional)", help="Describe what success looks like.")
        ...
        if not active_habits_dict:
            ...
            for habit_name, details in sorted_habits:
                ...
                with c2:
                    ...
                    habit_desc = details.get("description", "")

                    content = f"**{habit_name}** {level_badge} <br> <span style='background-color:#f0f2f6; color:#31333f; padding:2px 6px; border-radius:4px; font-size:0.75em'>{habit_goal}</span> <span style='color:gray; font-size:0.8em'>({details['xp']} XP)</span>"

                    st.markdown(content, unsafe_allow_html=True)
                    if habit_desc:
                        st.caption(habit_desc)
